<script lang="ts">
    ...
    // Function to fetch and update the keybind information
    ...
    // Function to toggle binding mode
    function toggleBinding(event: MouseEvent) {
        event.stopPropagation();

        if (binding) {
            // If already binding and canceling, reset to unknown
    ...
    listen("keyboardKey", async (e: KeyboardKeyEvent) => {
        ...
        // Find the KEY or BIND setting in this module's settings
        const settingWithBind = configurable.value.find(
            (s) => s.valueType === "KEY" || s.valueType === "BIND",
        );
        if (settingWithBind) {
            ...
            // Update the module settings with the new keybind
    ...
    // Listen for keybind changes and update accordingly
...
>
    ...
    {#if expanded && configurable}
        <div class="settings">
            {#each configurable.value.filter((setting) => !(// Filter out the default bind thing, maybe we can also check if it's the first "Bind" setting we've found incase someone tries to make their own setting with the name "Bind"
                        (setting.name === "Bind" && (setting.valueType === "KEY" || setting.valueType === "BIND")))) as setting (setting.name)}
                <GenericSetting
                    {path}
                    bind:setting
